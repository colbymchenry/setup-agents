...
Agents cannot spawn other agents via the `Agent` tool — this is a Claude Code limitation. Instead, the orchestrator uses `.claude/scripts/run-agent.sh` to spawn agents. This wrapper:
- Loads the full agent definition from `.claude/agents/<name>.md` (system prompt, tool restrictions, model)
...
### Generate Agent Runner Script (ALWAYS generate this)

Write to `.claude/scripts/run-agent.sh`. Make it executable (`chmod +x`).

This script is how the orchestrator spawns agents. It wraps `claude --agent <name> -p` with:
- Real-time tool call feedback (streams `→ ToolName(detail)` to stderr)
- Clean text response on stdout
- Proper `--output-format stream-json --verbose` handling
...
claude --agent "$AGENT_NAME" -p "$PROMPT" "${FLAGS[@]}" \
      ...
      for (const block of content) {
        if (block.type === 'tool_use') {
          ...
          if (input.file_path) {
            ...
            detail = cmd + (input.command.length > 80 ? '...' : '');
